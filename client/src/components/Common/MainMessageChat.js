...
const MainMessageChat = (props) => {
	...
	const sendMessage = (event) => {
		...
		axios
			.post("/api/chats/" + props.chat._id + "/messages", data, config)
			.then((newMessage) => {
				if (newMessage) {
					getMessages();
					setLoading(true);
	...
	const getMessages = () => {
		...
		axios
			.get("/api/chats/" + props.chat._id + "/messages")
			.then((newMessages) => {
				...
				<div className="row padding-16 scrollable" id="scrollable-div">
					{messages.length > 0
						? messages.map((msg, index) => {
								let text;
								msg.author.id === auth.currUser._id ? (text = <Message text={msg.content} key={index} />) : (text = <ResponseMessage text={msg.content} username={msg.author.username} key={index} />);
								return text;
						  })
						: null}
				</div>
				<form className="padding-32 chat-input-field-position" onSubmit={sendMessage}>
