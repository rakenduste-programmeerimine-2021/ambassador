	...
	let deletePopup =
		deleteUserPopup === true ? (
			<Popup clearPopupState={() => clearPopuState()}>
				...
				<button className="contact-button-delete margin-xs" onClick={() => removeContact()}>
					Delete
				</button>
			</Popup>
		) : null;
		...
		visible === true ? (
			<Popup clearPopupState={() => clearPopuState()}>
				...
				<form onSubmit={editContact}>
					...
					<button onClick={() => setVisibility(false)} className="add-nickname-cancel">
						Cancel
					</button>
				</form>
			</Popup>
		...
		return (
			<div className="user-list-item padding-20 row">
				...
				<div className="user-list-img-col clickable" onClick={() => props.selectContact(props.id)}>
					<svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="user" className="svg-inline--fa fa-user fa-w-14 svg-avatar-nav" role="img" viewBox="0 0 448 512">
						<path fill={color} d="M224 256c70.7 0 128-57.3 128-128S294.7 0 224 0 96 57.3 96 128s57.3 128 128 128zm89.6 32h-16.7c-22.2 10.2-46.9 16-72.9 16s-50.6-5.8-72.9-16h-16.7C60.2 288 0 348.2 0 422.4V464c0 26.5 21.5 48 48 48h352c26.5 0 48-21.5 48-48v-41.6c0-74.2-60.2-134.4-134.4-134.4z"></path>
					</svg>
				</div>
				...
				<div className="contact-buttons">
					<button className="contact-button-edit" onClick={() => setVisibility(true)}>
						Edit
					</button>
					<button className="contact-button-delete" onClick={deletePopupHandler}>
						Delete
					</button>
				</div>
			</div>
				...
				{props.type === "USER_LIST_GROUP" ? (
					...
					<div className="user-list-button-col">
						{props.alreadyAdded === "Already a friend" ? (
							...
							<button onClick={apiCall} id={props.name} className="contact-button-edit">
								Add friend
							</button>
						)}
					</div>
